...
public class ValidateIPAddress_468 {
    ...
    private String checkIPv4(String IP) {
        ...
        for (String addressPart :addressParts) {
            if (addressPart.length() <= 0
                    ...
                    || addressPart.startsWith("0") && addressPart.length() > 1 ) {
                return NEITHER;
            }
            int value = 0;
            for (char c : addressPart.toCharArray()) {
                if (c < '0' || c > '9') {
                    return NEITHER;
                }
                value = value * 10 + (c - '0');
    ...
    private String checkIPv6(String IP) {
        ...
        for (String addressPart : addressParts) {
            if (addressPart.length() <= 0 || addressPart.length() > 4) {
                return NEITHER;
            }
            for (char c : addressPart.toCharArray()) {
                char lowerChar = Character.toLowerCase(c);
                if (!((lowerChar >= '0' && lowerChar <= '9') || (lowerChar >= 'a' && lowerChar <= 'f'))) {
                    return NEITHER;
                }
            }
        }
